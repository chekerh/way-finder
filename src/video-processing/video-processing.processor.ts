    ...
    try {
      ...
      if (response.videoUrl && response.videoUrl.trim().length > 0) {
        ...
        try {
          ...
          this.logger.log(`Video generation completed for journey ${job.data.journeyId}: ${response.videoUrl}`);
        } catch (urlError) {
          this.logger.error(`Invalid video URL format: ${response.videoUrl}`, urlError);
          journey.video_status = 'failed';
          journey.video_url = undefined;
        }
      } else {
        // If no valid URL is returned, mark as failed
        this.logger.warn(`No valid video URL returned for journey ${job.data.journeyId}`);
        journey.video_status = 'failed';
        journey.video_url = undefined;
      }
      await journey.save();
      ...
      this.logger.error(`Video generation failed for journey ${job.data.journeyId}: ${errorMessage}`, error instanceof Error ? error.stack : undefined);
      journey.video_status = 'failed';
