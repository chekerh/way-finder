...
export class VideoProcessingProcessor {
  ...
  async handleGenerateVideo(job: Job<VideoJobPayload>) {
    ...
    if (!journey) {
      this.logger.warn(
        `Journey ${job.data.journeyId} not found, skipping video generation`,
      );
      return;
    }
    ...
    try {
      this.logger.log(
        `Starting video generation for journey ${job.data.journeyId} with ${job.data.slides.length} images`,
      );
