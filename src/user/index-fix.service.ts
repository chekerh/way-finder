    ...
    try {
      ...
      if (googleIdIndex) {
        ...
        try {
          ...
          if (
            dropError.code === 27 ||
            dropError.message?.includes('index not found')
          ) {
            this.logger.log("Index already dropped or doesn't exist.");
          } else {
            this.logger.warn(
              'Failed to drop index, will try to create anyway:',
              dropError.message,
            );
          }
        }
        ...
        this.logger.log(
          'No google_id_1 index found. Creating new sparse index...',
        );
      }

      // Check for existing users with null google_id before recreating index
      const usersWithNullGoogleId = await collection.countDocuments({
        google_id: null,
      });
      this.logger.log(
        `Found ${usersWithNullGoogleId} users with null google_id.`,
      );

      // Force index recreation - create it explicitly with sparse: true
      this.logger.log(
        'Creating google_id_1 index with unique: true, sparse: true...',
      );
      try {
        ...
        if (
          ...
          this.logger.error(
            '❌ Cannot create sparse index: duplicate null values exist.',
          );
          this.logger.error(
            'This should not happen with sparse index. Checking index status...',
          );
          // Try to get more info
          const allIndexes = await collection.indexes();
          this.logger.error(
            'All indexes:',
            JSON.stringify(allIndexes, null, 2),
          );
        }
        throw createError;
      }
      ...
      const newGoogleIdIndex = newIndexes.find(
        (idx) => idx.name === 'google_id_1',
      );
      if (newGoogleIdIndex) {
        if (newGoogleIdIndex.sparse) {
          this.logger.log(
            '✅ Verified: google_id_1 index is now sparse and working correctly.',
          );
        } else {
          this.logger.error(
            '❌ ERROR: google_id_1 index was created but is NOT sparse!',
          );
          this.logger.error(
            'Index details:',
            JSON.stringify(newGoogleIdIndex, null, 2),
          );
        }
      } else {
      ...
      if (error.code === 27 || error.message?.includes('index not found')) {
        this.logger.log(
          'Index does not exist. Mongoose will create it with sparse: true.',
        );
        return;
      }
      throw error;
