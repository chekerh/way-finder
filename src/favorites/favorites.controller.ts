...
export class FavoritesController {
  ...
  @Post()
  async addFavorite(@Req() req: any, @Body() createFavoriteDto: CreateFavoriteDto) {
    return this.favoritesService.addFavorite(req.user.sub, createFavoriteDto);
  }

  @Get()
  async getFavorites(@Req() req: any, @Query('type') itemType?: FavoriteItemType) {
    return this.favoritesService.getFavorites(req.user.sub, itemType);
  }

  @Get('count')
  async getFavoriteCount(@Req() req: any, @Query('type') itemType?: FavoriteItemType) {
    const count = await this.favoritesService.getFavoriteCount(req.user.sub, itemType);
    return { count };
  }
  ...
  async checkFavorite(
    ...
    const isFavorite = await this.favoritesService.isFavorite(req.user.sub, itemType, itemId);
    return { isFavorite };
  }
