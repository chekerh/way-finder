...
export class WeatherService {
  ...
  async getWeatherForecast(cityName: string, date?: Date): Promise<any> {
    ...
    try {
      ...
      this.logger.debug(
        `Fetching weather for: ${cityName} (using city: ${cityOnly}) on date: ${targetDate.toISOString()}`,
      );
      ...
      if (daysDifference >= 0 && daysDifference <= 5) {
        ...
        for (const forecast of forecasts) {
          ...
          const timeDiff = Math.abs(
            forecastTime.getTime() - targetDate.getTime(),
          );
        ...
        this.logger.debug(`Weather forecast retrieved for ${cityOnly}`);
        ...
        this.logger.debug(`Current weather retrieved for ${cityOnly}`);
      ...
      this.logger.error(
        `Error fetching weather from API: ${error.response?.data || error.message}`,
        error.stack,
      );
      this.logger.warn('Falling back to mock weather data');
  ...
  private getMockWeatherData(cityName: string, date?: Date): any {
    ...
    if (!data) {
      ...
      this.logger.debug(`Generated mock weather for ${cityName}`);
    }

    this.logger.debug(`Using mock weather data for ${cityName}`);
